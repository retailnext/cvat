...
def dump_as_cvat_interpolation(dumper, annotations):
    ...
    for shape in annotations.shapes:
        frame_step = annotations.frame_step if isinstance(annotations, TaskData) else annotations.frame_step[
            shape.task_id]
        if isinstance(annotations, TaskData):
            stop_frame = int(annotations.meta['task']['stop_frame'])
        else:
            task_meta = \
            list(filter(lambda task: int(task[1]['id']) == shape.task_id, annotations.meta['project']['tasks']))[0][1]
